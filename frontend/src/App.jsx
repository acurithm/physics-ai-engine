...
function App() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#000', color: '#fff', overflow: 'hidden', position: 'relative' }}>
      {/* Starry Background */}
      {stars.map((s, i) => <div key={i} style={{ position: 'absolute', top: s.top, left: s.left, width: s.size, height: s.size, background: 'white', borderRadius: '50%', zIndex: 0, opacity: 0.3 }} />)}

      {/* Sidebar */}
      <div style={{ width: sidebarOpen ? '260px' : '0px', transition: '0.3s', background: 'rgba(15,15,15,0.95)', zIndex: 100, borderRight: '1px solid #333', padding: sidebarOpen ? '20px' : '0', overflow: 'hidden', position: window.innerWidth < 768 ? 'absolute' : 'relative' }}>
        <h2 style={{ fontSize: '18px', marginBottom: '20px' }}>Recent Chats</h2>
        <button onClick={() => setChatHistory([])} style={{ background: '#333', color: '#fff', border: 'none', padding: '8px', borderRadius: '5px', width: '100%', cursor: 'pointer' }}>Clear History</button>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', zIndex: 1 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          {chatHistory.length === 0 && <h1 style={{ marginTop: '20vh', textAlign: 'center' }}>Let's crack JEE together!</h1>}
                    ...
                    code({node, inline, className, children, ...props}) {
                      ...
                      return !inline ? (
                        <div style={{ position: 'relative' }}>
                          <button onClick={() => navigator.clipboard.writeText(children)} style={{ position: 'absolute', right: 5, top: 5, fontSize: '10px', background: '#444', color: '#fff', border: 'none', cursor: 'pointer', padding: '2px 5px' }}>Copy</button>
        ...
        <div style={{ padding: '20px', display: 'flex', justifyContent: 'center' }}>
          <div style={{ width: '100%', maxWidth: '800px', display: 'flex', alignItems: 'flex-end', background: '#1a1a1a', borderRadius: '25px', padding: '10px 20px', border: '1px solid #333' }}>
